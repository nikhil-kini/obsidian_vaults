...
**Docker** is a set of platform as a service (PaaS) products that use OS-level virtualization to deliver software in packages called _containers_. The software that hosts the containers is called **Docker Engine**.
...
COMMAND | DESCRIPTION
---|---
`docker build -t [name:tag] .`| Builds image using Dockerfile located in same folder
`docker build -t [name:tag] -f [fileName]`| Builds image using Dockerfile located in same folder
`docker tag [imageName] [name:tag]`| Tags an existing image
...
`docker login` | Logins to a repository, by default Docker Hub
...
`docker run -it IMAGE -- /bin/bash` | To start a container and attach a shell to that container

**Container Management:**
*CONTAINER NAME can also be used instead of CONTAINER ID, ID is more specific *

COMMAND | DESCRIPTION
---|---
`docker create IMAGE` | Create a new container
`docker start CONTAINER ID` | Start a container
`docker stop CONTAINER ID` | Graceful stop a container
`docker kill CONTAINER ID` | Kill (SIGKILL) a container
`docker restart CONTAINER ID` | Graceful stop and restart a container
`docker pause CONTAINER ID` | Suspend a container
`docker unpause CONTAINER ID` | Resume a container
`docker rm CONTAINER ID` | Destroy a container
`docker container exec -it CONTAINER ID -- bash` | To attach to already running container
...
`docker inspect CONTAINER ID` | Show information of a container (json formatted)
...
`docker image inspect IMAGE` | Get info on the Image
`docker system prune -a` | To delete all the Images that are not in use
